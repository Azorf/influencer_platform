            ...
            <div className="relative h-48 bg-gradient-to-br from-gray-700 to-gray-900">
              ...
              <div className="absolute -bottom-12 left-6">
                {influencer.profilePictureUrl ? (
                  <img src={influencer.profilePictureUrl} alt={influencer.fullName || 'Profile'} className="w-24 h-24 rounded-full border-4 border-white object-cover" />
                ) : (
                  <div className="w-24 h-24 bg-gray-300 rounded-full border-4 border-white flex items-center justify-center text-3xl font-bold text-gray-600">{(influencer.fullName || influencer.username || '?').charAt(0).toUpperCase()}</div>
                )}
              </div>
            </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-xl font-bold text-gray-900">{influencer.fullName || influencer.username || 'Unknown'}</h2>
                  ...
                  <tr key={influencer.id} className={`hover:bg-gray-50 ${selectedInfluencers.includes(influencer.id) ? 'bg-gray-50' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setSelectedProfileId(influencer.id)} className="flex items-center gap-3 text-left hover:opacity-75">
                        {influencer.profilePictureUrl ? (
                          <img src={influencer.profilePictureUrl} alt={influencer.fullName || 'Profile'} className="w-10 h-10 rounded-full object-cover" />
                        ) : (
                          <div className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center text-gray-600 font-medium text-sm">{(influencer.fullName || influencer.username || '?').charAt(0).toUpperCase()}</div>
                        )}
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-medium text-gray-900 hover:underline">{influencer.fullName || influencer.username || 'Unknown'}</span>
